...
public abstract class Program implements Runnable
{
	...
	{
		session.setLastOperation(new WorkerOperation("",WorkerOperation.DATATYPE.STRING,s));
	}
	
	protected void SetResult(WorkerOperation op)
	{
		session.setLastOperation(op);
	}
	
	public void setExitSequence(String exitSequence)
	{
		this.exitSequence = exitSequence;
	}

	public String getExitSequence()
	{
		return exitSequence;
	}
	
	public void resetExitSequence()
	{
		exitSequence="exit";
	}
	...
	{
		if(s==null||exitSequence==null)
		{
			return false;
		}
		// TODO: Implement this method
		s = s.toLowerCase();
		if (s.trim().compareTo(exitSequence) == 0)
		{
			OnDestroy();
			return true;
		}
