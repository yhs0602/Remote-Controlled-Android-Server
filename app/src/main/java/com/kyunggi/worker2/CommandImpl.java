...
import com.kyunggi.worker2.ExternalProgram.*;
import java.io.*;
...
public class CommandImpl
{
	...
	public void DoQRStr(String[] args)
	{
		...
		if(s==null)
			s="not found or error";
		try
		{
			session.sender.Send(typeConverter.GetBatchInfo("", TypeConverter.FILETYPE.VNT, s));
		}
		catch (IOException e)
		{}
	}
	
	public void DoOnFileReceived(String [] args)
	{
		String s;
		session.setLastOperation(new WorkerOperation(args[0],WorkerOperation.DATATYPE.PATH,s=FinishArgs(args)));
		try
		{
			session.sender.Send(typeConverter.GetBatchInfo("", TypeConverter.FILETYPE.VNT, s));
		}
	...
	public void DoPs(String [] args)
	{
		...
		try
		{
			session.sender.Send(typeConverter.GetBatchInfo("", TypeConverter.FILETYPE.VNT, b.toString()));
		}
		catch (IOException e)
		{}
	}
	...
	public void DoExec(String [] args) throws NullPointerException,ArrayIndexOutOfBoundsException, IOException
	{
		...
		if(program==null)
		{
			...
			else if(args[1].compareToIgnoreCase("ocr")==0)
			{
				session.addProgram(new ProgramOCR(session));
			}
			else
			{
