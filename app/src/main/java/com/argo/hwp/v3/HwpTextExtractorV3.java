...
public abstract class HwpTextExtractorV3 {
	...
	private static String TAG="BTOPP HAN";
	...
	public static boolean extractText(File source, Writer writer)
		...
		try {
			...
			try {
				...
				Log.w("파일정보 확인 중 오류. HWP 포맷이 아닌 것으로 간주함", e);
				return false;
			}
			...
			try {
				...
				Log.w("exception while file.close", e);
			}
		}
	}
		...
		while (n_chars_read < n_chars) {
			...
			default:
				if (c >= 0x0020 && c <= 0xffff) {// # hnc code range
					...
					if (s == null) {
						Log.w(TAG,("매핑 문자 없음 {}"+Integer.toHexString(c)));
					...
					Log.e(TAG,"특수 문자 ? : {}"+ Integer.toHexString(c));
