...
public abstract class HwpTextExtractorV5 {
	...
	private static String TAG="BTOPP HAN V5";
	...
	public static boolean extractText(File source, Writer writer)
		...
		try {
			...
			try {
				...
				Log.w(TAG,"파일정보 확인 중 오류. HWP 포맷이 아닌 것으로 간주함", e);
				return false;
			}
	...
	private static FileHeader getHeader(NPOIFSFileSystem fs) throws IOException {
		...
		Log.d("Flags={}", Long.toBinaryString(flags).replace(' ', '0'));
			...
			if (entry.getName().startsWith("Section")
				...
				try {
					...
					try {
						...
						Log.e(TAG,"있을 수 없는 일?", e);
					}
				}
			} else {
				Log.w(TAG,"알수없는 Entry '{}'({})"+ entry.getName()+entry);
			}
		}
	}
	...
	private static Key readKey(InputStream input) throws IOException {
		...
		// Log.d(TAG,("TAG:   {}", recordHeader & 0x3FF);
		// Log.d(TAG,("LEVEL: {}", (recordHeader >> 10) & 0x3FF);
		// Log.d(TAG,("SIZE:  {}", (recordHeader >> 20) & 0xFFF);
	...
	private static void extractText(HwpStreamReader sectionStream, Writer writer)
		...
		while (true) {
			...
			if (buf.length() > 0) {
				Log.d(TAG,("TAG[{}]({}):{} [{}]" +new Object[] { tag.id,
	...
	private static void writeParaHeader(HwpStreamReader sectionStream,
		...
		// Log.d(TAG,("text={}", sectionStream.uint32());
		// Log.d(TAG,("control mask={}", sectionStream.uint32());
		// Log.d(TAG,("문단모양아이디참조값={}", sectionStream.uint16());
		// Log.d(TAG,("문단스타일아이디참조값={}", sectionStream.uint8());
		// Log.d(TAG,("단나누기종류={}", sectionStream.uint8());
		// Log.d(TAG,("글자모양정보수={}", sectionStream.uint16());
		// Log.d(TAG,("range tag정보수={}", sectionStream.uint16());
		// Log.d(TAG,("각줄에 대한 align정보수={}", sectionStream.uint16());
		// Log.d(TAG,("문단 Instance ID={}", sectionStream.uint32());
	...
	private static boolean readTag(HwpStreamReader sectionStream, TagInfo tag)
		...
		// Log.d(TAG,("Record Header={} [{}]", recordHeader,
